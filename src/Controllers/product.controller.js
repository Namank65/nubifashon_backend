...
const addProduct = asyncHandler(async (req, res) => {
  ...
  if (
    [name, images, category, newPrice, oldPrice, size].some(
      (field) => field?.trim() === ""
    )
  ) {
  ...
  return res
    ...
    .json(
      new apiResponse(
        200,
        createdProduct,
        "Producted Created And Added Successfully "
      )
    );
});
...
const allProducts = asyncHandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json(new apiResponse(200, allProduct, "All Product Fetched Successfully"));
});
  ...
  return res
    ...
    .json(
      new apiResponse(
        201,
        product,
        "All Latested products Fecthed SuccessFully"
      )
    );
});
  ...
  return res
    ...
    .json(
      new apiResponse(
        201,
        category,
        "All Unique Categories Fecthed SuccessFully"
      )
    );
});
...
const getSingleProduct = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new apiResponse(201, product, "Project Fecthed SuccessFully"));
});
...
const newCollection = asyncHandler(async (req, res) => {
  ...
  if (!newCollection)
    throw new apiError(400, "Error in Fetching NewCollection");

  return res
    .status(200)
    .json(
      new apiResponse(201, newCollection, "New Collection Fecthed SuccessFully")
    );
});
...
const popularInWomen = asyncHandler(async (req, res) => {
  ...
  if (!popularInWomen)
    throw new apiError(400, "Error in Fetching Popular In Women");
  ...
  return res
    ...
    .json(
      new apiResponse(
        201,
        popularInWomen,
        "Popular In Women Fetched Sucessfully"
      )
    );
});
...
const removeFromCart = asyncHandler(async (req, res) => {
  ...
  if (userData.cartData[req.body.itemId].quantity >= 1) {
    userData.cartData[req.body.itemId].quantity = 0;
  ...
  return res
    ...
    .json(
      new apiResponse(
        201,
        userData.cartData,
        "Fetched All User's Cart Data Successfully"
      )
    );
});
